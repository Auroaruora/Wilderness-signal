...
func rerender_positions(cell_positions: Array):
	for cell_position in cell_positions:
		set_cell(0, cell_position, 0, Vector2i(min(calculated_lightmap[cell_position], MAX_LIGHT), 0))
...
func remove_light(cell_position: Vector2i):
	...
	for clear_position in cleared_cells:
		propagate_light_at_position(clear_position)
